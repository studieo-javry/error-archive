...
class OutboxCleanupJob(
    ...
    @SchedulerLock(name = "iam-outbox-cleanup", lockAtMostFor = "PT10M", lockAtLeastFor = "PT30S")
    @Transactional
    fun purge() {
        val cutoff = Instant.now().minus(Duration.ofDays(properties.sentTtlDays))
        val deleted = outboxRepo.deleteSentBefore(cutoff)
        if (deleted > 0) {
            log.info { "iam outbox cleanup: deleted $deleted SENT rows older than ${properties.sentTtlDays}d" }
        }
    }
}
