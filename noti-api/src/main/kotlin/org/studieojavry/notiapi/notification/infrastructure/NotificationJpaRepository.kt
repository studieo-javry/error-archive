...
interface NotificationJpaRepository : JpaRepository<NotificationEntity, Long> {
    ...
    // SSE catchup — `id > sinceId` 인 row.
    fun findAllByRecipientUserIdAndIdGreaterThanOrderByCreatedAtDescIdDesc(
        recipientUserId: Long, sinceId: Long, pageable: Pageable,
    ): List<NotificationEntity>

    fun findAllByRecipientUserIdAndIdGreaterThanAndReadAtIsNullOrderByCreatedAtDescIdDesc(
        recipientUserId: Long, sinceId: Long, pageable: Pageable,
    ): List<NotificationEntity>
