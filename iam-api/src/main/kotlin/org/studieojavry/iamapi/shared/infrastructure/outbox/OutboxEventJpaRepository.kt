...
interface OutboxEventJpaRepository : JpaRepository<OutboxEventEntity, Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM iam_outbox_event WHERE status = 'PENDING'", nativeQuery = true)
    fun countPending(): Long

    @Query(value = "SELECT COUNT(*) FROM iam_outbox_event WHERE status = 'DEAD'", nativeQuery = true)
    fun countDead(): Long

    @Query(value = "SELECT MIN(created_at) FROM iam_outbox_event WHERE status = 'PENDING'", nativeQuery = true)
    fun oldestPendingCreatedAt(): Instant?

    @Modifying
    @Query(
        value = "DELETE FROM iam_outbox_event WHERE status = 'SENT' AND sent_at < :before",
        nativeQuery = true,
    )
