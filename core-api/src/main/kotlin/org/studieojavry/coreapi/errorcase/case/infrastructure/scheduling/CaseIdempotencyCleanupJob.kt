package org.studieojavry.coreapi.errorcase.case.infrastructure.scheduling
...
/**
 * 케이스 생성 멱등성 기록 정리 — 48시간 지난 row 삭제.
 *
 * 멱등 키는 클라이언트 재시도 창(수 분~수 시간) 동안만 유효하면 되므로 48h 면 충분.
 * 매일 03:20 KST 실행, shedlock 으로 멀티 노드에서 1회만.
 */
@Component
class CaseIdempotencyCleanupJob(
    private val jpa: CaseIdempotencyJpaRepository,
) {
    private val log = KotlinLogging.logger {}

    @Scheduled(cron = "0 20 3 * * *", zone = "Asia/Seoul")
    @SchedulerLock(name = "case-idempotency-cleanup", lockAtMostFor = "PT10M", lockAtLeastFor = "PT30S")
    @Transactional
    fun purge() {
        val cutoff = Instant.now().minus(RETENTION)
        val deleted = jpa.deleteCreatedBefore(cutoff)
        if (deleted > 0) {
            log.info { "case idempotency cleanup: deleted $deleted rows older than ${RETENTION.toHours()}h" }
        }
    }
