 ...
 * `noti.publisher.mode=kafka` 일 때만 활성화. 그 외엔 NoopActivityEventPublisherAdapter fallback.
 */
@Component
...
class OutboxActivityEventPublisherAdapter(
    ...
    override fun publish(event: ActivityEventPublisherPort.ActivityEvent) {
        try {
            ...
            log.warn(ex) { "iam outbox activity INSERT failed (silently dropped): key=${event.idempotencyKey}" }
        }
    }
